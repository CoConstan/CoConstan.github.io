---
layout: page
title: Operational Research
description: Linear programming and graph theory
img: assets/img/RO.jpg
importance: 2
category: Polytech
department: MAM - Applied Maths and Modeling
academic_year: 2024–2026
related_publications: False
---

<h2>Some useful background videos:</h2>
<p><span style="text-decoration: underline;">Linear programming:</span></p>
<ul>
<li><a href="https://www.youtube.com/watch?v=E72DWgKP_1Y" target="_blank" rel="noopener noreferrer">The Art of Linear Programming</a> by Tom S.</li>
<li><a href="https://www.youtube.com/watch?v=9YKLXFqCy6E" target="_blank" rel="noopener noreferrer">Intro to Simplex Method | Solve LP | Simplex Tableau</a> by Joshua Emmanuel</li>
<li><a href="https://www.coursera.org/lecture/algorithms-part2/simplex-algorithm-JJQmi" target="_blank" rel="noopener noreferrer">Princeton University's Simplex Algorithm course</a> on Coursera</li>
</ul>
<p><span style="text-decoration: underline;">Graph theory:</span></p>
<ul>
<li><a href="https://www.youtube.com/watch?v=LFKZLXVO-Dg" target="_blank" rel="noopener noreferrer">Introduction to Graph Theory : A Computer Science Perspective</a> by Reducible</li>
<li><a href="https://www.youtube.com/watch?v=PMMc4VsIacU" target="_blank" rel="noopener noreferrer">Depth First Search (DFS) Explained: Algorithm, Examples, and Code</a> by Reducible</li>
<li>The whole <a href="https://www.youtube.com/playlist?list=PLDV1Zeh2NRsDGO4--qE8yH72HFL1Km93P" target="_blank" rel="noopener noreferrer">Graph Theory Playlist</a> by William Fiset</li>
</ul>

<h2 id="mcetoc_1icgqd2gd5a">TD (2024-2025) : </h2>
<ul>
<li><a href="{{ site.baseurl }}/assets/pdf/RO/RO_TD1.pdf">TD1</a></li>
<li><a href="{{ site.baseurl }}/assets/pdf/RO/RO_TD2.pdf">TD2</a></li>
<li><a href="{{ site.baseurl }}/assets/pdf/RO/RO_TD3_4.pdf">TD3/4</a></li>
<li><a href="/assets/pdf/RO/RO TD Simplexe.pdf" target="_blank" rel="noopener noreferrer">TD5</a>, <a href="/assets/pdf/RO/Exo1 corrigé.odp" target="_blank" rel="noopener noreferrer">Exo1 Correction</a>,  <a href="/assets/pdf/RO/Exo2 corrigé.odp" target="_blank" rel="noopener noreferrer">Exo2 Correction</a> </li>

<!-- <a href="/assets/pdf/RO/Exo1 corrigé.odp" target="_blank" rel="noopener noreferrer">Exo1 Correction</a>,  <a href="/assets/pdf/RO/Exo2 corrigé.odp" target="_blank" rel="noopener noreferrer">Exo2 Correction</a></li> -->

<li><a href="/assets/pdf/RO/TD_theory_de_graph.pdf" target="_blank" rel="noopener noreferrer">TD6</a>, <a href="/assets/pdf/RO/TD_theory_de_graph _correction.pdf" target="_blank" rel="noopener noreferrer">Correction</a> </li>

<!-- , <a href="/assets/pdf/RO/TD_theory_de_graph _correction.pdf" target="_blank" rel="noopener noreferrer">Correction</a></li> -->
<li><a href="/assets/pdf/RO/TD Dijkstra.pdf" target="_blank" rel="noopener noreferrer">TD7</a>, <a href="/assets/pdf/RO/Dijkstra corrigé.odp" target="_blank" rel="noopener noreferrer">Correction</a> </li>

<!-- , <a href="/assets/pdf/RO/Dijkstra corrigé.odp" target="_blank" rel="noopener noreferrer">Correction</a></li> -->
</ul>

<h2 id="mcetoc_1icgqd2gd5b">TP :</h2>
<ul>
<li><a href="{{ site.baseurl }}/assets/pdf/RO/RO_TP.pdf">TP1 sujet</a>, <a href="{{ site.baseurl }}/assets/files/RO/TP1 RO.c">Correction en C</a>, <a href="{{ site.baseurl }}/assets/files/RO/TP_1_RO_Python.zip">Correction en Python</a> </li>
<li><a href="/assets/pdf/RO/TP Branch and bound.pdf" target="_blank" rel="noopener noreferrer">TP2 sujet</a>, TP2 Correction</li>
</ul>
